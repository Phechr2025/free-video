...
def search():
    ...
    cur = conn.execute(
        ...
        SELECT *,
        CASE
          WHEN lower(title) = lower(?) THEN 4             -- ชื่อตรง 100%
          WHEN lower(title) LIKE lower(?) THEN 3          -- มีทั้งประโยคค้นหา
          WHEN lower(title) LIKE lower(?) THEN 2          -- มีคำหลัก (เช่น ชื่อเรื่องหลัก)
          WHEN lower(description) LIKE lower(?) THEN 1    -- เจอในคำอธิบาย
          ELSE 0
        END AS score
        FROM series
        WHERE
        ...
        (
            query,           # เทียบเท่ากัน
            like_full,       # LIKE ทั้งประโยค
            like_main,       # LIKE คำหลัก
            like_main,       # description
            like_full,       # WHERE title LIKE ทั้งประโยค
            like_main,       # WHERE title LIKE คำหลัก
            like_main,       # WHERE description LIKE คำหลัก
        ),
    )
