...
@app.route("/watch/<int:series_id>/<int:episode_id>")
def watch_episode(series_id, episode_id):
    ...
    # sqlite3.Row ไม่มี .get() ต้องใช้ keys()
    series_active = 1
    if "is_active" in series.keys():
        try:
            series_active = int(series["is_active"]) if series["is_active"] is not None else 1
        except Exception:
            series_active = 1
    ...
    if "is_active" in episode.keys():
        try:
            episode_active = int(episode["is_active"]) if episode["is_active"] is not None else 1
        except Exception:
            episode_active = 1
...
def stream_episode(episode_id):
    ...
    # ตรวจสอบสถานะเปิด/ปิดจากฐานข้อมูล
    series_active = 1
    if "is_active" in series.keys():
        try:
            series_active = int(series["is_active"]) if series["is_active"] is not None else 1
    ...
    # ถ้าเรื่องหรืออตอนถูกปิด → ห้ามสตรีม
