...
@app.route("/series_blocked")
def series_blocked():
    # หน้าแจ้งเตือนเมื่อเรื่องถูกปิดการให้ดูชั่วคราว
    return render_template("series_blocked.html")
...
def watch_episode(series_id, episode_id):
    ...
    # ตรวจสอบสถานะเปิด/ปิดของ "เรื่อง"
    series_active = 1
    try:
    ...
    # ถ้าเรื่องถูกปิด → ไปหน้าแจ้งเตือน แล้วให้ผู้ใช้กด "ตกลง" เพื่อย้อนกลับหน้าหลัก
    if series_active == 0:
        return redirect(url_for("series_blocked"))

    # ตรวจสอบสถานะเปิด/ปิดของ "ตอน"
    episode_active = 1
    try:
    ...
    # ถ้าเรื่องเปิดปกติ แต่ตอนถูกปิด → ใช้ระบบ blocked เดิม
    blocked = (episode_active == 0)
