...
def _before_every_request():
    # ทำ global logout วันละครั้ง (หลังเที่ยงคืนเวลาไทย)
    try:
        global_logout_if_midnight()
    except Exception:
        pass

    # ถ้าผู้ใช้ล็อกอินอยู่ ให้ตรวจสอบว่า session ยัง active
    if session.get("user_id") and session.get("session_token"):
        try:
            if not validate_current_session():
                # session ถูกบังคับออก
                logout_user()
                # ถ้าเป็น API JSON ให้ตอบ 401
                if request.path.startswith("/account/sessions"):
                    return ({"ok": False, "error": "session_expired"}, 401)
                # หน้าเว็บทั่วไป
                flash("บัญชีนี้ถูกออกจากระบบแล้ว (อาจเกิดจากการบังคับออก/เที่ยงคืน)", "info")
...
def create_user_session(user_id: int, user_agent: str, ip: str) -> str:
    token = uuid.uuid4().hex
    platform = detect_platform(user_agent)
    now_iso = _now_thai_iso()
    conn = get_db_connection()
    ensure_session_tables(conn)
    conn.execute(
        """
        INSERT INTO user_sessions (user_id, session_token, user_agent, ip, platform, created_at, last_seen, is_active)
        VALUES (?, ?, ?, ?, ?, ?, ?, 1)
        """,
        (user_id, token, user_agent or "", ip or "", platform, now_iso, now_iso),
    )
    conn.commit()
    conn.close()
    return token

def deactivate_session_token(token: str):
    if not token:
        return
    conn = get_db_connection()
    ensure_session_tables(conn)
...
def deactivate_all_sessions_for_user(user_id: int, except_token: str | None = None):
    conn = get_db_connection()
    ensure_session_tables(conn)
    if except_token:
        conn.execute(
...
def global_logout_if_midnight():
    """ล็อกเอาท์ทุก session วันละครั้งหลัง 00:00 ตามเวลาไทย (ทำงานเมื่อมี request เข้ามาหลังเที่ยงคืน)."""
    conn = get_db_connection()
    ensure_session_tables(conn)
    today = datetime.now(TH_TZ).date().isoformat()
    last = meta_get(conn, "last_global_logout_date")
    if last != today:
        # ทำ global logout: ปิดทุก session
        conn.execute("UPDATE user_sessions SET is_active = 0")
        meta_set(conn, "last_global_logout_date", today)
    conn.commit()
    conn.close()

def validate_current_session() -> bool:
    """คืนค่า True ถ้า session ปัจจุบันยัง active"""
    ...
    ensure_session_tables(conn)
    row = conn.execute(
        "SELECT is_active FROM user_sessions WHERE user_id = ? AND session_token = ?",
        (user_id, token),
    ).fetchone()
    if not row:
        conn.close()
        return False
    active = row["is_active"] if isinstance(row, sqlite3.Row) else row[0]
    if active == 1:
        conn.execute(
            "UPDATE user_sessions SET last_seen = ? WHERE user_id = ? AND session_token = ?",
            (_now_thai_iso(), user_id, token),
        )
        conn.commit()
        conn.close()
        return True
...
def login_user(user_row):
    ...
    # สร้าง session token เพื่อแยกอุปกรณ์/การล็อกอิน
    token = create_user_session(user_row["id"], request.headers.get("User-Agent", ""), request.remote_addr)
    session["session_token"] = token


def logout_user():
    # ปิด session ปัจจุบันในฐานข้อมูล (ถ้ามี)
    token = session.get("session_token")
    if token:
        try:
            deactivate_session_token(token)
        except Exception:
            pass
