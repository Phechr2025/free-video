...
def admin_series():
    ...
    if request.method == "POST":
        ...
        if not title:
            ...
            # ขั้นแรก: เพิ่มเรื่องโดยยังไม่รู้ path ปกจากไฟล์
            cur = conn.cursor()
            cur.execute(
                """
            ...
            # ถ้ามีไฟล์รูปจากมือถือ → เซฟลง static/covers/series_<id>/
            if cover_file and cover_file.filename:
                ...
                rel_path_from_static = os.path.join("covers", f"series_{series_id}", safe_name)
                thumbnail_value = rel_path_from_static

            # ถ้าไม่มีไฟล์ แต่มีลิงก์รูปจากเน็ต → ใช้ลิงก์แทน
            elif thumbnail_url_input:
                thumbnail_value = thumbnail_url_input

            # อัปเดตค่า thumbnail_url ให้ series แถวนี้
    ...
    if request.method == "POST":
        ...
        conn.execute(
            ...
            INSERT INTO episodes (
                ...
                source_type, video_url, drive_id, file_path, created_at
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
...
def admin_delete_episode(episode_id):
    ...
    conn = get_db_connection()
    ep = conn.execute(
        "SELECT id, series_id, file_path FROM episodes WHERE id = ?",
        (episode_id,),
    ).fetchone()
    if ep is None:
        conn.close()
