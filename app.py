...
# เรียกสร้างตารางทันทีตอนโหลดโมดูล (รองรับ Flask 3.x)
init_db()
...
def admin_upload():
    ...
    if request.method == "POST":
        ...
        # ----- Mode B: Google Drive + gdown -----
            ...
            try:
                file_path_real = download_drive_file(drive_id)
            except Exception as e:
                flash(str(e), "error")
                return redirect(url_for("admin_upload"))

            # เก็บ path แบบ relative เพื่อกัน path ผิดเมื่อ deploy
            rel_path = os.path.relpath(file_path_real, os.path.dirname(__file__))
            file_path = rel_path
            source_type = "gdrive"
        ...
        # ----- Mode C: อัปโหลดไฟล์ mp4 จากเครื่อง -----
            ...
            filename = os.path.basename(filename)  # ป้องกัน path แปลก ๆ
